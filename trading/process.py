...
def log_transaction(transaction_type, date, stock, number_of_shares, price, fees, ledger_file):
    ...
    '''
    # Check which transaction type is input
    if transaction_type == 'buy':
        # Calculate the total amount of money paid
        money = - ((price)*number_of_shares+fees)
   
    elif transaction_type == 'sell':
        # Calculate the total amount of money earned (probably)
        money = (price)*number_of_shares-fees
    else:
        # When wrong transaction type is input
        print('Please enter a real transaction type:(')
        return None

    # Write a string that will be logged into the ledger
    text = transaction_type+','+str(date)+','+str(stock)+','+str(number_of_shares)+','+'{:.2f}'.format(price)+','+'{:.2f}'.format(money)
    # Write the string into the ledger file
...
def buy(date, stock, available_capital, stock_prices, fees, portfolio, ledger_file):
    ...
    '''
    # Get the stock price of the stock on the specific day
    price = stock_prices[date, stock]
    
    # Calculate how many shares can be bought
    shares = (available_capital - fees) // price
    # Calculate how much money is spent
    money = - (shares*price+fees)
    
    # Update the portfolio
    portfolio[stock] += shares
    
    # Log transactions
    text = 'buy,'+str(date)+','+str(stock)+','+str(shares)+','+'{:.2f}'.format(price)+','+'{:.2f}'.format(money)+'\n'
    # Write into the ledger file
...
def sell(date, stock, stock_prices, fees, portfolio, ledger_file):
    ...
    # Get the number of shares in my portfolio
    ...
    # Log transactions
    text = 'sell,'+str(date)+','+str(stock)+','+str(shares)+','+'{:.2f}'.format(price)+','+'{:.2f}'.format(money)+'\n'
    # Write into the ledger file
    with open(ledger_file, 'a') as ledger:
        ledger.write(text)
...
def create_portfolio(available_amounts, stock_prices, fees, ledger_file):
    ...
    '''
    # Get the stock price of each stock on day 0
    prices = stock_prices[0,:]
    # Change the object type to list
    prices = prices.tolist()
    
    # Create a list to store shares I purchased for each stock
    portfolio = []
    
    with open(ledger_file, 'w') as ledger:
    # Loop over every element in prices and available_amounts
        for i in range(len(prices)):
            # Calculate how many shares I can buy for each stock
            share = (available_amounts[i] - fees) // prices[i] 
            share = int(share)
            # Update the portfolio
            portfolio.append(share)
            # Calculate the money used
            money = - (share*prices[i]+fees)
            # Log transactions
            text = 'buy,'+'0'+','+str(i)+','+str(share)+','+'{:.2f}'.format(prices[i])+','+'{:.2f}'.format(money)+'\n'
            # Write into the ledger file
            ledger.write(text)
